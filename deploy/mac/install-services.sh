...
# daemon API is HTTPS with a local CA — verify against the CA (strict pinning).
# The CA is generated by the daemon on its FIRST boot, so wait for it to exist
# before probing (a fresh install's verify would otherwise always fail).
# /health requires the Bearer token whenever one is configured (loopback
# included) — read it from config.json; a token-less install probes unauthed.
API_TOKEN=$(/usr/bin/python3 -c "import json,sys; print(json.load(open('$DATA_DIR/config.json')).get('api',{}).get('token') or '')" 2>/dev/null || true)
verify_https() {
  local url="$1" name="$2" i
  local auth=()
  [ -n "$API_TOKEN" ] && auth=(-H "Authorization: Bearer $API_TOKEN")
  for i in $(seq 1 20); do
    if [ ! -f "$DATA_DIR/certs/ca.crt" ]; then sleep 0.5; continue; fi
    if curl -fsS --cacert "$DATA_DIR/certs/ca.crt" "${auth[@]}" -o /dev/null "$url" 2>/dev/null; then echo "✓ $name up ($url)"; return 0; fi
    sleep 0.5
  done
  echo "✗ $name not responding at $url — check logs under $DATA_DIR"; return 1
}
verify_https "https://127.0.0.1:3737/health" "daemon API" || true
